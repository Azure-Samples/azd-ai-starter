...
final_result_format = "# AI Gallery Standard Validation: {{result}} \n The Definition of Done of the AI-Gallery repo can be found [Here]({link}). \n {{message}}".format(
    link=details_help_link)


class ItemResultFormat:
    PASS = "<details><summary>{sign} <b>{{message}}</b>.</summary></details>".format(
        sign=Signs.CHECK)
    FAIL = "<details><summary>{sign} <b>{{message}}</b>. <a href={detail_link}>[Detail]</a></summary>\n\n{{detail_messages}}\n\n</details>".format(
        sign=Signs.BLOCK, detail_link=details_help_link)
    WARNING = "<details><summary>{sign} <b>{{message}}</b>. <a href={detail_link}>[Detail]</a></summary>\n\n{{detail_messages}}\n\n</details>".format(
